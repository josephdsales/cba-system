...
$st = db()->prepare("SELECT e.*, u.fullname AS teacher_name,
    ...
    (SELECT 1 FROM exam_students es WHERE es.exam_id=e.id AND es.student_id=?) AS remedial_assigned,
    (SELECT 1 FROM attempts a2 JOIN exam_students es2 ON es2.exam_id=a2.exam_id AND es2.student_id=a2.student_id
        WHERE a2.exam_id=e.id AND a2.student_id=? AND a2.submitted_at IS NOT NULL) AS remedial_taken
...
$st->execute([$user['id'], $user['id'], $user['id'], $user['section_id'], $user['id']]);
$exams = $st->fetchAll();
...
<div class="card"><div class="table-wrap"><table>
  ...
  <tr>
    ...
    <td><?php if ($x['done_id']): ?>
      ...
      <?php if (!empty($x['allow_retake']) && !empty($x['remedial_assigned']) && empty($x['remedial_taken'])): ?>
